...
export function toControls(room, userId, obj) {
  let entregues = 0;
  if (debug) console.log(`[diag] toControls — userId:${userId} tipo:${obj.type}, controles na sala: ${room.controles.size}`);
  for (const ws of room.controles) {
    if (ws.__controlOf !== userId) continue;
    if (sendJson(ws, obj)) entregues++;
  }
  if (debug) console.log(`[diag] toControls — ${entregues} entrega(s) para ${userId}`);
  return entregues;
}
...
export function needKeyframe(room, ws, slot) {
  ...
  if (entry?.streaming) requestKeyframe(entry, { urgente: true });
}
...
export function watch(room, ws, slot, rtc = true) {
  const entry = room.slots.get(slot);
  ...
  if (rtc !== false) sendJson(entry.ws, { type: 'rtc-want', peer: ws.__peerId });
