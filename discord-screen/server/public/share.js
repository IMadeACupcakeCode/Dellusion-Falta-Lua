...
import {
  ...
  fonteIndisponivel,
  opcoesTela,
} from '/shared/broadcaster.js?v=27';
...
function atenderPedido(fonte, novas) {
  console.log('[diag] share atenderPedido:', fonte, 'painel:', !!paineis[fonte], 'ativo:', paineis[fonte]?.ativo(), 'indisponivel:', paineis[fonte]?.indisponivel());
  aplicarOpcoes(novas);
...
function ligarControle() {
  ...
  const url = `${proto}://${location.host}/ws?t=${encodeURIComponent(token)}&modo=controle`;
  console.log('[diag] share controle — conectando:', url);
  controle = new WebSocket(url);

  controle.addEventListener('open', () => console.log('[diag] share controle — CONECTADO'));
  ...
  controle.addEventListener('message', (e) => {
    ...
    if (msg.type === 'start-request') {
      console.log('[diag] share controle — RECEBEU start-request:', msg.fonte, msg.opcoes);
  ...
  controle.addEventListener('close', () => {
    console.log('[diag] share controle — DESCONECTADO');
    controle = null;
...
function criarPainel(fonte) {
  ...
  async function ligar() {
    ...
    console.log('[diag] share ligar() — fonte:', fonte, 'token:', token ? 'presente' : 'AUSENTE');
    ...
    console.log('[diag] share ligar — wsUrl:', wsUrl, 'previa:', !!previa);
    ...
    broadcaster = createBroadcaster({
      ...
      onStatus: (s) => {
        console.log('[diag] share onStatus:', s);
        setStatus(
      ...
      onEnd: (reason) => {
        console.log('[diag] share onEnd:', reason);
        broadcaster = null;
        mostrarSetup();
        setStatus(reason);
      },
    ...
    try {
      console.log('[diag] share — chamando broadcaster.start()...');
      const stream = await broadcaster.start();
      console.log('[diag] share — broadcaster.start() OK, stream:', !!stream);
      ...
      console.error('[diag] share — broadcaster.start() ERRO:', err.name, err.message);
      broadcaster = null;
      el('start').disabled = false;
