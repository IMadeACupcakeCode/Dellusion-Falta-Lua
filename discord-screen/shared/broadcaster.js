...
export function createBroadcaster({
  ...
  // ICE pode chegar antes do answer pela sinalização em WebSocket. A fila é
  // por peer para não misturar candidatos de espectadores diferentes.
  const icePendente = new Map(); // peerId -> RTCIceCandidateInit[]
  ...
  async function start() {
    console.log('[diag] broadcaster start() — fonte:', fonte, 'streamPronto:', !!streamPronto);
    ...
    console.log('[diag] broadcaster start() — stream obtido, tracks:', stream.getTracks().length);
    ...
    console.log('[diag] broadcaster start() — config:', config.codec, `${config.width}x${config.height}`);

    await connect();
    console.log('[diag] broadcaster start() — WebSocket conectado');
    ...
    console.log('[diag] broadcaster start() — encoder configurado, estado:', encoder.state);

    ws.send(JSON.stringify({ type: 'start' }));
    console.log('[diag] broadcaster start() — mensagem start enviada');
  ...
  function onEncoded(chunk, metadata) {
    ...
    if (frames === 0) console.log('[diag] broadcaster — primeiro chunk codificado, tipo:', chunk.type, 'bytes:', chunk.byteLength);
  ...
  function connect() {
    return new Promise((resolve, reject) => {
      console.log('[diag] broadcaster connect() — abrindo WebSocket:', wsUrl);
      ws = new WebSocket(wsUrl);
      ws.binaryType = 'arraybuffer';
      let slotRecebido = false;

      const timeout = setTimeout(() => {
        ws.close();
        reject(new Error('O servidor não atribuiu um slot de transmissão a tempo.'));
      }, 10_000);

      ws.addEventListener('open', () => {
        console.log('[diag] broadcaster WebSocket ABERTO');
      });
      ...
      ws.addEventListener('close', () => {
        clearTimeout(timeout);
        if (!slotRecebido) reject(new Error('A conexão fechou antes de receber o slot.'));
  ...
  async function receberRtc(peerId, payload) {
    ...
    try {
      if (payload.kind === 'answer' && payload.sdp) {
        ...
        const candidatos = icePendente.get(peerId) ?? [];
        icePendente.delete(peerId);
        for (const candidate of candidatos) await pc.addIceCandidate(candidate);
      } else if (payload.kind === 'ice' && payload.candidate) {
        if (!pc.remoteDescription) {
          const candidatos = icePendente.get(peerId) ?? [];
          if (candidatos.length < 128) candidatos.push(payload.candidate);
          icePendente.set(peerId, candidatos);
        } else {
