...
/** A transmissão continua até o transmissor pedir para parar. */
{
  ...
  viewer.close();
  console.log('confirmando que nao existe limite de inatividade (22s)...');
  await new Promise((r) => setTimeout(r, 22000));

  const pedido = doTx.find((m) => m.type === 'stop-request');
  check(
    'quem saiu da sala nao encerra a transmissao',
    !pedido && !tx.fechou,
    'recebeu: ' + JSON.stringify(doTx.map((m) => m.type)),
  );
  check(
    'a transmissao continua sem pedido de parada',
    !pedido,
