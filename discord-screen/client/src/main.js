...
// Activities não oferecem um transporte WebRTC confiável; o relay por
// WebSocket é o caminho suportado pelo Discord. O site normal continua usando
// WebRTC quando o navegador oferece a API.
const podeUsarWebRTC = !inDiscord && suportaWebRTC();
...
function comPrazo(promise, ms, message) {
  let timer;
...
function watchSlot(slot) {
  ...
  if (!info) return;
  watching.add(slot);
  ws?.send(JSON.stringify({ type: 'watch', slot, rtc: podeUsarWebRTC }));
...
function openStream(slot, userId) {
  ...
  const s = {
    ...
    viaRtc: false,
    prazoRtc: null,
    prazoDesconexao: null,
    // Candidatos podem chegar antes da oferta ou enquanto ela é aplicada.
    icePendente: [],
    ...
    player: createPlayer(canvas, {
      ...
      onNeedKeyframe: () => {
        if (watching.has(slot)) ws?.send(JSON.stringify({ type: 'need-keyframe', slot }));
      },
      onTamanho: () => {
        s.started = true;
        renderGrid();
...
async function receberOferta(slot, sdp) {
  ...
  try {
    ...
    const pc = criarPeer({
      ...
      onEstado: (estado) => {
        clearTimeout(s.prazoDesconexao);
        s.prazoDesconexao = null;
        if (MORTO.has(estado)) {
          desistirDoRtc(slot);
        } else if (estado === 'disconnected') {
          // Uma perda breve de rota é recuperável. Só devolve ao relay depois
          // de uma janela, para evitar trocar de transporte a cada oscilação.
          s.prazoDesconexao = setTimeout(() => {
            if (s.pc?.connectionState === 'disconnected') desistirDoRtc(slot);
          }, 4000);
        }
      },
      onTrack: (e) => {
    ...
    for (const candidate of s.icePendente.splice(0)) await pc.addIceCandidate(candidate);
...
async function receberIce(slot, candidate) {
  ...
  if (!pc || !pc.remoteDescription) {
    if (s.icePendente.length < 128) s.icePendente.push(candidate);
    return;
  }
  try {
...
async function boot() {
  ...
  try {
    ...
    clearTimeout(vigia);
    setEmpty('Não foi possível conectar', err.message);
    throw err;
  }
...
async function authDiscord(fonteDoId) {
  ...
  try {
    await comPrazo(sdk.ready(), 12_000, 'O Discord não respondeu ao iniciar a Activity.');
  } catch (erro) {
    const config = typeof fonteDoId === 'string' ? {} : await fonteDoId;
    const origem = config.publicOrigin || '(domínio público configurado no servidor)';
    throw new Error(
      `${erro.message} O endereço *.discordsays.com é interno do Discord e não ` +
        `deve ser usado como Target. Use "${origem.replace(/^https?:\/\//, '')}" ` +
        'em Activities → URL Mappings e abra a Activity novamente.',
    );
  }

  const { code } = await comPrazo(sdk.commands.authorize({
    client_id: clientId,
    response_type: 'code',
    state: '',
    prompt: 'none',
    ...
    scope: ['identify'],
  }), 12_000, 'O Discord não respondeu à autorização da Activity.');
  ...
  const [, sessao] = await Promise.all([
    comPrazo(
      ...
      12_000,
      'O Discord não confirmou a sessão da Activity.',
    ),
    post(`${P}/api/session`, {
      access_token,
...
function connect() {
  ...
  ws.addEventListener('message', (e) => {
    ...
    if (msg.type === 'state') {
      ...
      console.log('[diag] ws state — participants:', participants.length, 'abas:', [...abas], 'streams:', (msg.streams ?? []).map(s => `slot${s.slot}:${s.userId}`));
      ...
      console.log('[diag] ws stream-start — slot:', msg.slot, 'userId:', msg.userId, 'fonte:', msg.fonte);
...
function ligarFonte(fonte) {
  console.log('[diag] ligarFonte:', fonte, 'abaAberta:', abaAberta());
...
function trazerAba(fonte) {
  console.log('[diag] trazerAba:', fonte, 'inDiscord:', inDiscord);
...
async function abrirCaptura(fonte) {
  if (!roomTokens) return;
  console.log('[diag] abrirCaptura:', fonte, 'inDiscord:', inDiscord);

  // Só a tela tem chance de nascer aqui dentro; o Discord anula o getUserMedia
  // no iframe, então a câmera vai direto para a aba.
  if (fonte === 'tela' && (await broadcastFromHere())) {
    console.log('[diag] abrirCaptura — broadcastFromHere retornou true, saindo');
    return;
  }
  console.log('[diag] abrirCaptura — broadcastFromHere retornou false, indo para abrirLink');
...
async function abrirLink(fonte) {
  ...
  console.log('[diag] abrirLink:', url);
...
$('share').addEventListener('click', () => {
  ...
  console.log('[diag] share click — minhasFontes:', [...minhasFontes()], 'myBroadcast:', !!myBroadcast, 'abaAberta:', abaAberta());
...
async function broadcastFromHere() {
  console.log('[diag] broadcastFromHere — getDisplayMedia:', !!navigator.mediaDevices?.getDisplayMedia, 'VideoEncoder:', !!window.VideoEncoder);
  ...
  const proto = location.protocol === 'https:' ? 'wss' : 'ws';
  const wsUrl = `${proto}://${location.host}${P}/ws?t=${encodeURIComponent(shareToken)}`;
  console.log('[diag] broadcastFromHere — wsUrl:', wsUrl);
  ...
  const b = createBroadcaster({
    ...
    fps: ajustes.fps,
    audio: true,
    onAviso: (m) => { console.log('[diag] broadcastFromHere — aviso:', m); toast(m, true); },
    onEnd: (reason) => { console.log('[diag] broadcastFromHere — onEnd:', reason); myBroadcast = null; renderBar(); },
  });

  const startedAt = performance.now();
  try {
    console.log('[diag] broadcastFromHere — chamando b.start()...');
    await b.start();
    console.log('[diag] broadcastFromHere — b.start() OK, myBroadcast definido');
    myBroadcast = b;
    renderBar();
    return true;
  } catch (err) {
    console.error('[diag] broadcastFromHere — ERRO:', err.name, err.message);
    const showedPicker = performance.now() - startedAt > 250;
    // Mudança: se der NotAllowedError depois que o seletor apareceu, NÃO retornamos true.
    // Deixamos cair para abrir a aba externa (que você já disse que funciona).
    // O retorno true antigo fazia o Activity "pensar" que broadcasting começou, mas myBroadcast
    // nunca era definido, deixando tudo em silêncio sem erro.
    if (err.name === 'NotAllowedError' && showedPicker) {
      console.warn('[diag] broadcastFromHere — NotAllowedError pós-seletor, caindo para aba externa');
      return false;  // <-- mudou de return true para return false
    }
    return false;
  }
}
